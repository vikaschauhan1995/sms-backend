...
const updateAttendanceOfStudent = async (student_id, created_date, is_present) => {
  ...
  const getAttendanceQuery = `UPDATE student_attendance SET ${student_attendance_table?.IS_PRESENT} = $1 WHERE ${student_attendance_table?.STUDENT_ID} = $2 AND ${student_attendance_table?.CREATED_DATE} = $3 RETURNING *`;
