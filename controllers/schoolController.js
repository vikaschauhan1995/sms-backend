...
const createSchool = async (req, res) => {
  ...
  try {
    ...
    const query = `INSERT INTO school (${school.SCHOOL_ID}, ${school.SCHOOL_NAME}, ${school.MOBILE_NUMBER}, ${school.ADDRESS1}, ${school.ADDRESS2}, ${school.STATE}, ${school.PINCODE}, ${school.COUNTRY}, ${school.EXPIRATION_DATE}) VALUES ($1,$2,$3,$4,$5,$6,$7,$8,$9) RETURNING *`;
    const newSchool = await db.query(query, [school_id, school_name, mobile_number, address1, address2, state, pincode, country, expiration_date]);
    res.status(200).json(newSchool?.rows[0]);
  } catch (err) {
...
const getSchools = async (req, res) => {
  try {
    const query = `SELECT * FROM school ORDER BY id DESC`;
...
const deleteSchool = async (req, res) => {
  ...
  try {
    ...
    res.status(200).json(deletedSchool.rows?.[0]);
  } catch (err) {
    res.status(400).json({ error: err.message });
  }
}
